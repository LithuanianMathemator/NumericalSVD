...
def householder_bidiag(A, U, V_t, mode='real', compute='USV'):
    ...
    for j in range(min(m_B, n_B)):
        ...
        if mode == 'complex' and j > 0 and (j-1 <= n_B-2 or n_B > m_B):
                
            theta = np.angle(B[j-1,j])
            B[j-1:,j] = np.exp(-1j*theta)*B[j-1:,j]
            if compute == 'SV' or compute == 'USV':
                V_t[j,:] = np.exp(1j*theta)*V_t[j,:]
    ...
    if mode == 'complex' and n_B > m_B:
                
        theta = np.angle(B[m_B-1,m_B])
        B[m_B-1,m_B] = np.exp(-1j*theta)*B[m_B-1,m_B]
        if compute == 'SV' or compute == 'USV':
            V_t[m_B,:] = np.exp(1j*theta)*V_t[m_B,:]

    if compute == 'S':
        return abs(np.diag(B)), abs(np.diag(B,1))
    if compute == 'US':
        return U, abs(np.diag(B)), abs(np.diag(B,1))
    if compute == 'SV':
        return abs(np.diag(B)), abs(np.diag(B,1)), V_t
    if compute == 'USV':
        return U, abs(np.diag(B)), abs(np.diag(B,1)), V_t
...
def determine_householder(x, mode='real'):
    ...
    if mode == 'complex':
        ...
        if sigma == 0 and (theta <= np.pi/2 or theta > -np.pi/2):
            return v, 0
        elif sigma == 0 and (theta > np.pi/2 or theta <= -np.pi/2):
            return v, 2
        else:
